...
/**
 * Скрипт для регистрации user03 (Боб) с низким gas price
 */
...
async function registerUser03LowGas() {
    try {
        console.log('📝 Регистрация user03 (Боб) с низким gas price...');
        ...
        // Регистрируем пользователя с низким gas price
        const tx = contract.methods.registerPublicKey(publicKeyHex);
        const gas = await tx.estimateGas({ from: address });
        
        // Используем более низкий gas price
        const gasPrice = '10000000000'; // 10 gwei вместо 20
        const totalCost = BigInt(gas) * BigInt(gasPrice);
        
        console.log(`⛽ Gas: ${gas}`);
        console.log(`💰 Gas Price: ${gasPrice} wei (10 gwei)`);
        console.log(`💸 Общая стоимость: ${web3.utils.fromWei(totalCost.toString(), 'ether')} ETH`);
